...
namespace HMS.Model.Operations
{
    public class Checkout1
    {
        ...
        public DataTable CC()
        {
            var list = new List<SqlParameter>();
            list.AddSqlParameter("@ROOM_NO", RR);
            string S = "UPDATE CHECKIN SET CHECK_OUT=1 WHERE ROOM_NO ='" + RR + "' AND CHECKIN_ID IN (SELECT Max(CHECKIN_ID) FROM CHECKIN WHERE ROOM_NO ='" + RR + "' ) ";
        ...
        public DataTable Room_NA_Details()
        {
            ...
            var list = new List<SqlParameter>();
            string s = "SELECT c.INSERT_DATE,c.INSERT_TIME,c.ROOM_TARRIF,c.ROOM_NO,(SELECT AMOUNT_RECEIVED FROM ADVANCE WHERE ADVANCE = 0 AND CHECKIN_ID = c.CHECKIN_ID AND INSERT_DATE = c.INSERT_DATE) AS ADVANCE,(select CHARGES from POSTCHARGES where POSTCHARGES = 0 AND  CHECKIN_ID = c.CHECKIN_ID AND INSERT_DATE = c.INSERT_DATE) AS CHARGES,c.GST FROM night_audit c WHERE NIGHT = 0 AND CHECKIN_ID = (Select CHECKIN_ID from CHECKIN where ROOM_NO = '" + ROOM_NO + "' AND CHECK_OUT = 0)";
            DataTable dt = DbFunctions.ExecuteCommand<DataTable>(s, list);
            return dt;
        }
